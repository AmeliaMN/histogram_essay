...
Network = () ->
  ...
  update = () ->
    ...
    # radial layout
    if layout == "radial"
      colors = sortedcolors(curNodesData, curLinksData)
      updateCenters(colors)
  ...
  network.setGrouping = (newGrouping) ->
    if newGrouping == "material"
    	groupBy = (d) -> d.material
    else if newGrouping == "brand"
    	groupBy = (d) -> d.brand
    else if newGrouping == "size"
    	groupBy = (d) -> d.size
    else if newGrouping = "length"
    	groupBy = (d) -> d.length
    else
    	groupBy = (d) -> d.color
    update()
  ...
  setupData = (data) ->
    ...
    countExtent = d3.extent(data.nodes, (d) -> d.length)
    ...
    data.nodes.forEach (n) ->
      ...
      n.radius = circleRadius(n.length)
  ...
  # attr is value stored in node, like 'color'
  ...
  filterNodes = (allNodes) ->
    ...
    if filter == "popular" or filter == "obscure"
      lengths = allNodes.map((d) -> d.length).sort(d3.ascending)
      ...
      filteredNodes = allNodes.filter (n) ->
        if filter == "popular"
          ...
          n.length <= cutoff
    else if filter == "washable"
      	filteredNodes = allNodes.filter (n) -> n.wash == true
    else if filter == "nowashable"
    	filteredNodes = allNodes.filter (n) -> n.wash == false
    else if filter == "lined"
    	filteredNodes = allNodes.filter (n) -> n.lined == true
    else if filter == "nolined"
    	filteredNodes = allNodes.filter (n) -> n.lined == false
    else if filter == "pockets"
    	filteredNodes = allNodes.filter (n) -> n.pockets == true
    else if filter == "nopockets"
    	filteredNodes = allNodes.filter (n) -> n.pockets == false
    else if filter == "petite"
    	filteredNodes = allNodes.filter (n) -> n.petite == true  
    else if filter == "nopetite"
    	filteredNodes = allNodes.filter (n) -> n.petite == false  	
    filteredNodes

  # Returns array of colors sorted based on
  # current sorting method.
  sortedcolors = (nodes,links) ->
    colors = []
    if sort == "links"
      counts = {}
      links.forEach (l) ->
        ...
        counts[groupBy(l.source)] ?= 0
        counts[groupBy(l.source)] += 1
      # add any missing colors that dont have any links
      nodes.forEach (n) ->
        counts[groupBy(n)] ?= 0

      # sort based on counts
      colors = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      # get just names
      colors = colors.map (v) -> v.key
    else
      # sort colors by song count
  ...
  showDetails = (d,i) ->
    content = '<p class="main">' + "color: #{d.color}" + '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + "#{d.length} inches long" + '</span></p>'
    content += '<hr class="tooltip-hr">'
    isIt = (d) ->
    	if d == false
    		return("not")
    ...
    content += '<p class="main">' + "made of #{d.material}"+ '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + "#{isIt(d.lined)} lined"+ '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + "does #{isIt(d.pockets)} have pockets"+ '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + "#{isIt(d.wash)} machine washable"+ '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + "#{isIt(d.dry)} dryable"+ '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + "made of #{d.material}"+ '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + "#{isIt(d.petite)} petite"+ '</span></p>'
    content += '<hr class="tooltip-hr">'
    getImage = (d) ->
    	return("img/#{d.id}.jpg")
