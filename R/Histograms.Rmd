...
```{r, eval=FALSE}
manipulate(
  ggplot(minidiamonds) + geom_histogram(aes(x=price),bins = binnum, center=centering, closed=ifelse(side==TRUE, "right", "left")),
...
ggplot(diamonds) + geom_histogram(aes(x=carat), bins=15, col='green')
...
### Playing with marathon finishing time data

```{r, message=FALSE, warning=FALSE}
library(readr)
library(dplyr)
master_marathon <- read_csv("~/Downloads/master_marathon.csv")
master_marathon <- master_marathon %>%
  mutate(hours = chiptime/60)
nyc_marathon <- master_marathon %>% 
  filter(marathon == "New York City Marathon", year == 2013)
p1 <- ggplot(nyc_marathon, aes(x=hours, y=(..count..)/50062))  
p1 + geom_histogram(bins=130, fill="grey70") + theme_classic()+ geom_vline(xintercept=seq(2, 7, by=0.25), color="white",size=0.25) +ylab("") +scale_x_continuous(breaks=seq(from=2, to=7, by=0.5), labels = c("2:00", "2:30", "3:00", "3:30", "4:00", "4:30", "5:00", "5:30", "6:00","6:30", "7:00"),limits = c(2,6.75),name = "")+scale_y_continuous(breaks=seq(from=0, to=0.02, by=0.01))
```
